...
describe('withTimeout', () => {
  it('resolves with the operation result when it finishes in time', async () => {
    await expect(
      withTimeout(Promise.resolve('pong'), 50, 'probe'),
    ).resolves.toBe('pong');
  });
  ...
  it('rejects with a TimeoutError once the deadline passes', async () => {
    ...
    await expect(
      withTimeout(never, 10, 'database probe'),
  ...
  it('names the operation in the timeout message so logs are actionable', async () => {
    ...
    await expect(withTimeout(never, 5, 'redis probe')).rejects.toThrow(
      'redis probe timed out after 5ms',
    );
  });
  ...
  it('propagates the original rejection rather than masking it as a timeout', async () => {
    ...
    await expect(withTimeout(failure, 1_000, 'probe')).rejects.toThrow(
      'connection refused',
    );
  });
